...
const Home = () => {
  ...
  const fetchNotes = async () => {
    try {
      ...
      const notesWithData = response.data.map(note => ({
        id: note._id,
  ...
  const handleCreateNote = async () => {
    try {
      await axios.post('http://localhost:3003/notes', { content: newNote });
      fetchNotes();
      setNewNote('');
    } catch (error) {
      console.error(error);
  ...
  const handleUpdateNote = async () => {
    try {
      await axios.put(`http://localhost:3003/notes/${editNote.id}`, { content: editNote.content });
      fetchNotes();
